...
export class LockSession {
    ...
    public static _from_object(_o: {
        [_K in keyof LockSession]: LockSession[_K];
    }): LockSession {
        return new LockSession(_o.entryName, _o.atribute);
    }
}
 ...
 * @constant
 */
export const _root_component_type_list_1_spec_for_LockSession: $.ComponentSpec[] =
    [
        new $.ComponentSpec(
            'entryName',
            false,
 ...
 * @constant
 */
export const _root_component_type_list_2_spec_for_LockSession: $.ComponentSpec[] =
    [];
 ...
 * @constant
 */
export const _extension_additions_list_spec_for_LockSession: $.ComponentSpec[] =
    [];
 ...
 */
export function _decode_LockSession(el: _Element) {
    if (!_cached_decoder_for_LockSession) {
        _cached_decoder_for_LockSession = function (el: _Element): LockSession {
            const sequence: _Element[] = el.sequence;
            if (sequence.length < 2) {
                throw new _ConstructionError(
                    'LockSession contained only ' +
            ...
            entryName = $._decode_explicit<DistinguishedName>(
                () => _decode_DistinguishedName
            )(sequence[0]);
            atribute = $._decode_explicit<OBJECT_IDENTIFIER>(
                () => $._decodeObjectIdentifier
            )(sequence[1]);
            return new LockSession(entryName, atribute);
        };
    }
 ...
 */
export function _encode_LockSession(
    value: LockSession,
    elGetter: $.ASN1Encoder<LockSession>
) {
    if (!_cached_encoder_for_LockSession) {
        _cached_encoder_for_LockSession = function (
            value: LockSession,
            elGetter: $.ASN1Encoder<LockSession>
        ): _Element {
