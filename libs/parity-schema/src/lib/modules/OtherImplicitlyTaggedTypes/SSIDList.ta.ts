...
import {
    ...
    _decode_SSID,
    _encode_SSID,
} from '../OtherImplicitlyTaggedTypes/SSID.ta';
 ...
 * ```
 */
export type SSIDList = SSID[]; // SequenceOfType
 ...
 */
export function _decode_SSIDList(el: _Element) {
    if (!_cached_decoder_for_SSIDList) {
        _cached_decoder_for_SSIDList = $._decodeSequenceOf<SSID>(
            () => _decode_SSID
        );
    }
 ...
 */
export function _encode_SSIDList(
    value: SSIDList,
    elGetter: $.ASN1Encoder<SSIDList>
) {
    if (!_cached_encoder_for_SSIDList) {
        _cached_encoder_for_SSIDList = $._encodeSequenceOf<SSID>(
            () => _encode_SSID,
            $.BER
        );
    }
