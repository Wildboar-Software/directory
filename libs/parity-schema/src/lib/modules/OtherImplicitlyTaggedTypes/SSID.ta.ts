 ...
 * ```
 */
export type SSID = OCTET_STRING; // OctetStringType
 ...
 */
export function _decode_SSID(el: _Element) {
    if (!_cached_decoder_for_SSID) {
        _cached_decoder_for_SSID = $._decodeOctetString;
    }
 ...
 */
export function _encode_SSID(value: SSID, elGetter: $.ASN1Encoder<SSID>) {
    if (!_cached_encoder_for_SSID) {
        _cached_encoder_for_SSID = $._encodeOctetString;
    }
