...
import { ObjectSel, _decode_ObjectSel, _encode_ObjectSel } from "../OtherImplicitlyTaggedTypes/ObjectSel.ta";
export { ObjectSel, _decode_ObjectSel, _encode_ObjectSel } from "../OtherImplicitlyTaggedTypes/ObjectSel.ta";
...
class AccessService {
     ...
     */
    public static _from_object (_o: { [_K in keyof (AccessService)]: (AccessService)[_K] }): AccessService {
        return new AccessService(_o.serviceId, _o.objectDef, _o._unrecognizedExtensionsList);
    }
 ...
 */
export
const _root_component_type_list_1_spec_for_AccessService: $.ComponentSpec[] = [
    new $.ComponentSpec("serviceId", false, $.hasTag(_TagClass.universal, 6), undefined, undefined),
    new $.ComponentSpec("objectDef", false, $.hasTag(_TagClass.universal, 16), undefined, undefined)
];
 ...
 */
export
const _root_component_type_list_2_spec_for_AccessService: $.ComponentSpec[] = [
 ...
 */
export
const _extension_additions_list_spec_for_AccessService: $.ComponentSpec[] = [
 ...
 */
export
function _decode_AccessService (el: _Element) {
    if (!_cached_decoder_for_AccessService) { _cached_decoder_for_AccessService = function (el: _Element): AccessService {
    const sequence: _Element[] = el.sequence;
    if (sequence.length < 2) {
        throw new _ConstructionError("AccessService contained only " + sequence.length.toString() + " elements.");
    }
    ...
    objectDef = $._decodeSequenceOf<ObjectSel>(() => _decode_ObjectSel)(sequence[1]);
 ...
 */
export
function _encode_AccessService (value: AccessService, elGetter: $.ASN1Encoder<AccessService>) {
    if (!_cached_encoder_for_AccessService) { _cached_encoder_for_AccessService = function (value: AccessService, elGetter: $.ASN1Encoder<AccessService>): _Element {
    return $._encodeSequence(([] as (_Element | undefined)[]).concat(
        [
            /* REQUIRED   */ $._encodeObjectIdentifier(value.serviceId, $.BER),
            /* REQUIRED   */ $._encodeSequenceOf<ObjectSel>(() => _encode_ObjectSel, $.BER)(value.objectDef, $.BER)
        ],
