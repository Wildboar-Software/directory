...
export class BPUReport {
    ...
    public static _from_object(_o: {
        [_K in keyof BPUReport]: BPUReport[_K];
    }): BPUReport {
 ...
 * @constant
 */
export const _root_component_type_list_1_spec_for_BPUReport: $.ComponentSpec[] =
    [
        new $.ComponentSpec(
            'contentType',
            false,
 ...
 */
export function _decode_BPUReport(el: _Element) {
    if (!_cached_decoder_for_BPUReport) {
        _cached_decoder_for_BPUReport = function (el: _Element): BPUReport {
            const sequence: _Element[] = el.sequence;
            if (sequence.length < 2) {
                throw new _ConstructionError(
                    'BPUReport contained only ' +
            ...
            return new BPUReport(contentType, content);
        };
    }
 ...
 */
export function _encode_BPUReport(
    value: BPUReport,
    elGetter: $.ASN1Encoder<BPUReport>
) {
    if (!_cached_encoder_for_BPUReport) {
        _cached_encoder_for_BPUReport = function (
            value: BPUReport,
            elGetter: $.ASN1Encoder<BPUReport>
        ): _Element {
