...
import {
    id_as_mta_rtse,
} from "@wildboar/x400/src/lib/modules/MHSProtocolObjectIdentifiers/id-as-mta-rtse.va";
import {
    id_ac_mts_transfer,
...
import {
    RTSE_apdus,
    _encode_RTSE_apdus,
} from "@wildboar/rtse/src/lib/modules/Reliable-Transfer-APDU/RTSE-apdus.ta";
import {
    RTORQapdu, RTORQapdu_dialogueMode_monologue,
...
const implementation_data: GeneralString = "@wildboar/x400-client";
...
const app_context_to_abstract_syntax: Map<IndexableOID, OBJECT_IDENTIFIER> = new Map([
    [ id_ac_mts_transfer.toString(), id_as_mta_rtse ],
]);
...
function rose_transport_from_itot_stack (itot: ISOTransportOverTCPStack): ROSETransport {
    ...
    itot.acse.outgoingEvents.on('AARQ', (apdu) => {
        itot.acse.presentation.request_P_CONNECT({
            presentation_context_definition_list: [
                ...
                new Context_list_Item(3, id_as_mta_rtse, [id_ber]),
                new Context_list_Item(5, id_as_mtse, [id_ber]),
            ],
            user_data: {
                fully_encoded_data: [
    ...
    rose.write_bind = (params) => {
        ...
        const rtpdu: RTSE_apdus = {
            rtorq_apdu: new RTORQapdu(
                63,
                undefined,
                RTORQapdu_dialogueMode_monologue,
                {
                    open: _encode_Bind({
                        bind_invoke: params.parameter,
                    }, BER),
                },
                undefined,
            ),
        };
        // TODO: Wrap in RTSE, which is required for MTS transfer.
