...
import { MasterAndShadowAccessPoints } from "@wildboar/x500/DistributedOperations";
...
async function updateNHOBSubordinateDSA (
    ...
    if ("result" in response) {
        ...
        if ("protected_" in response.result.parameter) {
            ...
            if ("signed" in param) {
                try {
                    ...
                    return; // The above must be awaited for this return.
                }
            }
        ...
        if (compareCode(response.error.code, securityError["&errorCode"]!)) {
            ...
            if ("signed" in param) {
                // We do not await, because we only log this failure, unbind, and return.
                verifySIGNED(
                    ctx,
                    undefined,
                    sp?.certification_path,
