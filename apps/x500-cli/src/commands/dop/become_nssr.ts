...
async function do_become_nssr (
    ...
    if (typeof argv["ae-title"] !== "string") {
        throw new Error("You must specify an AE-Title of the DSA with which the NHOB should be established.");
    }
    ...
    const sup2sub = new SuperiorToSubordinate(
        objectName.map((rdn) => new Vertex(rdn, undefined, undefined, undefined)),
        undefined,
        undefined,
    );
