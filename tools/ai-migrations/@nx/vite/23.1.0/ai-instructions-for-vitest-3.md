...
1. Run affected tests: `nx affected -t test`
2. Re-baseline coverage on key projects: `nx run <project>:test --coverage`
3. Typecheck custom reporters / sequencers / snapshot tooling: `nx affected -t typecheck`
4. Watch mode (if applicable): verify `server.watch.ignored` covers what `watchExclude` previously did.
